<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="设备编号" prop="deviceAddr">
        <el-input
    ...
    <el-tabs v-model="activeName" type="card" @tab-click="handleClick">
      <el-tab-pane label="数据表格" name="first">
        <el-table v-loading="loading" :data="wyList" :max-height="tableHeight" @selection-change="handleSelectionChange" border>
          ...
          <el-table-column label="水平位移（m）" align="center" prop="valueWyX" />
          <el-table-column label="垂直位移（m）" align="center" prop="valueWyY" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="水平位移" prop="valueWyX">
          <el-input v-model="form.valueWyX" placeholder="请输入水平位移" />
        </el-form-item>
        <el-form-item label="垂直位移" prop="valueWyY">
          <el-input v-model="form.valueWyY" placeholder="请输入垂直位移" />
        </el-form-item>
...
<script>
import { listWy, getWy, delWy, addWy, updateWy } from "@/api/aqy/sdrk";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除山东仁科位移数据编号为"' + ids + '"的数据项？').then(function() {
        return delWy(ids);
      }).then(() => {
        this.getList();
